...
> (SFT → 偏好对齐 → 奖励模型 → 在线 RL → 评测 → 量化部署)在一张 Blackwell 单卡上从头跑通。
...
- **硬件**:单张 Blackwell 卡(sm_120)。主线在 **molab** 的 RTX PRO 6000(96GB,预装 torch 2.12/cu130);
  备选 AutoDL 的 RTX 5090。环境配置见 [`env/README.md`](env/README.md)。

## 阶段索引

| Phase | 主题 | 脚本 | 状态 |
|---|---|---|---|
| [0](phase0_architecture/) | 看懂结构:ViT + connector + LLM | `env/setup_molab.sh`(AutoDL:`setup_autodl.sh`) | 🚧 脚本就绪,待验证 |
...
> 在 molab notebook 里,每条命令用 `!` 前缀在 cell 执行(或开一个 terminal 直接跑)。
> AutoDL 则在终端直接跑,并把 Phase 0 换成 `bash env/setup_autodl.sh`。
> `env/common.sh` 自动探测平台(`/marimo` → molab / `/root/autodl-tmp` → AutoDL),脚本两边通用。

```bash
# 0) 拉代码(molab 落持久盘 /marimo;AutoDL 落 /root/autodl-tmp)
cd /marimo && git clone https://github.com/rickliang-JY/RLStudio.git || git -C RLStudio pull
cd RLStudio/vlm_posttraining

# 0) Phase 0:环境验证 + 推理 sanity(不装训练框架,不碰预装 torch)
bash env/setup_molab.sh          # AutoDL: bash env/setup_autodl.sh
...
> 各阶段产物默认落在持久盘 `$DATA_ROOT/vlm/`(molab=`/marimo/vlm/`,AutoDL=`/root/autodl-tmp/vlm/`):
> adapter / merge 模型 / 日志 / 评测结果,路径与超参见 `env/common.sh`。
> molab 单 session 上限 12h,长阶段(如 GRPO)可分段续跑,产物落 `/marimo` 关机不丢。
...
- 显存充足(PRO 6000 96GB / 5090 32GB)跑 2B 全流程绰绰有余;7B 走 LoRA/QLoRA 可行,PPO/GRPO 配合 vLLM colocate 可跑。
